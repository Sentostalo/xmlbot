...
class XMLToolsCog:
	...
	async def fetch_reviews(self,client,first,automated):
		if automated:  # if this is an automated fetch called by the background task
				...
				try:
					...
					if (len(mapcrew_list) > 5):  # if the fetch ran successfully
						...
						await self.events_channel.send(
							'Mapcrew list failed to fetch. Reverted to previously saved edition.')
			...
			await self.events_channel.send(f'Reviews successfully fetched. Next review fetching scheduled in {datetime.timedelta(days=d.days,seconds=d.seconds)}')
		...
		try:
			...
			if category:
				...
				if category_name in reviews:
					await ctx.send(f'**Review for {category_name}** *(Fetched every two hours)*```\n{reviews[category_name]}```')
				else:
