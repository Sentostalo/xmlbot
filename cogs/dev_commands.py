...
class DevCommandsCog:
	...
	async def on_command_error(self,ctx,error):
		if (isinstance(error,discord.ext.commands.errors.CommandOnCooldown)):
			await ctx.send(f'```{str(error)}```', delete_after=2)
		else:
			print(error)
	...
	async def exit(self,ctx):
		...
		try:
			...
			await self.add_log(f'{ctx.author.nick} ({str(ctx.author)}) commanded {ctx.bot.user.name} exit from Channel #{ctx.message.channel.name} in {ctx.message.guild}. Total uptime: {up_time}\n\u200B')
		except Exception as e:
			print(f'Error in logout log output: {e}')
		await ctx.bot.logout()

	@commands.command(name='cleanlog',hidden=True)
	async def cleanlog(self,ctx):
		cleared = await ctx.bot.logs_channel.purge(limit=100, check=self.clean_log_check)
		await ctx.message.channel.send(
			content=f'**Clean log**```{len(cleared)} non-log messages found in my log channel and cleared.```', delete_after=2)
	...
	async def give_dev(self,ctx):
		...
		for my_guilds in ctx.bot.guilds:
			...
			if to_give != None and ctx.message.guild != my_guilds:
				try:
					...
					given.append(f'{my_guilds} is either not a dev server or I do not have the permissions to give you the dev role.')
	...
	async def cog_unload(self, ctx, *, cog: str):
		cog=self.find_cog(cog)
		dev_cog=str(os.path.basename(__file__)).replace('.py', '')
		if cog == (f'cogs.{dev_cog}'):
...
def setup(client):
	client.add_cog(DevCommandsCog(client))
